...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--keep_only_above", type=float, default=None)
...
def console_main() -> None:
    ...
    if args.keep_only_above is not None:
        kept_our_specs = []
        for spec in our_specs:
            if float(spec.id) >= args.keep_only_above:
                kept_our_specs.append(spec)
        print(f"Keep-only-above mode activated (val={args.keep_only_above}). "
              f"Keeping only {len(kept_our_specs)}/{len(our_specs)} specs.")
        print([spec.id for spec in kept_our_specs])
        our_specs = kept_our_specs
