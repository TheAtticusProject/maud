...
def main(
    ...
    for run in range(1, n_runs + 1):
        ...
        if args.eval_split == "valid":
            dev_df = data_utils.load_df(train=True)
            # ds, add_ds, synth_ds = data_utils.df_to_grouped_records(
            group = data_utils.df_to_grouped_records(
                dev_df,
                question=spec.answer_key,
                subquestion=spec.sub_question_key,
            )
            ds_ = group["main"] + group["counterfactual"] + group["additional"]
            print(f"{len(ds_)} dev records loaded from disk ({spec.n_classes} classes)")
            train_ds, test_ds = data.build_balanced_split_only(
                ds=(group["main"] + group["counterfactual"]),  # both sides
                valid_prop=args.valid_prop,
                verbose=False,
                add_ds=group["additional"],  # grouped by contract name.
                synth_ds=[],
                seed=420,
            )
        elif args.eval_split == "test":
            train_df = data_utils.load_df(train=True)
            test_df = data_utils.load_df(train=False)
            ...
            print(f"{len(train_ds)} dev records loaded from disk ({spec.n_classes} classes)")
...
def build_spec_id_to_best_tune_kwargs(df_best_hps, model_name: str) -> Dict[str, dict]:
    # Use output to filter by requested spec_id.
    model_mask = df_best_hps["model_name"] == model_name
    our_df_best = df_best_hps[model_mask]
    ...
    for i in tqdm.tqdm(range(n_rows)):
        ...
        args_text = (f"-m {model_name} --spec_id {spec_id} "
                     f"-l {learning_rate} --num_updates {num_updates}"
                     f"--eval_split test"
                     )
        parser = get_parser()
...
def console_main() -> None:
    ...
    if args.load_test_best_hps_path is not None:
        ...
        print(f"[SPECIAL] Attempting to load spec_ids={our_specs}")
        df_best_hps_path: pathlib.Path = args.load_test_best_hps_path
        assert df_best_hps_path.is_file()
        with open(df_best_hps_path, "rb") as f:
            df_best_hps = pickle.load(f)
        spec_id_to_best_kwargs = build_spec_id_to_best_tune_kwargs(df_best_hps, model_name=args.model_name)
